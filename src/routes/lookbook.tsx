            ...
            <FadeIn key={p.id}>
              <div className={`grid gap-6 md:grid-cols-12 md:gap-10 ${flip ? "md:[&>*:first-child]:order-2" : ""}`}>
                ...
                <div className="md:col-span-4 md:self-end md:pb-6">
                  ...
                  <h2 className="mt-4 font-display text-4xl leading-tight text-[var(--color-ink)]">{p.title}</h2>
                  <p className="mt-4 text-[var(--color-ink)]/70">{p.snippet}</p>
                  <div className="mt-6 text-[10px] uppercase tracking-[0.28em] text-[var(--color-taupe)]">{p.date}</div>
                </div>
              </div>
            </FadeIn>
          );
